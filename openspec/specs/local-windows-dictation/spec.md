# Specification: local-windows-dictation

## Purpose
Offline Windows dictation from fixed global hotkey through microphone capture, local Whisper transcription, and active text input insertion. Provides tray-first Tauri 2 application with `Ctrl + Alt + Space` toggle, local Turkish transcription via whisper.cpp, and clipboard-based text insertion without external services or history retention.
